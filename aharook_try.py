...
def default_panel(chat_id, notify=False):
    ...
    item2 = telebot.types.KeyboardButton("📢 Підписка")
    ...
    if str(user_id) == str(admin_id):
        if message.text == "/admin":
            ...
            if admin_start_time and not admin_shift_closed and admin_shift_date == today:
                ...
                notify_subscribers(f"Зміна почалася ({formatted_start})")
            ...
            if admin_start_time and not admin_on_break and not admin_shift_closed:
                ...
                notify_subscribers("Адмін пішов на перерву ⏸")
            else:
            ...
            if admin_on_break and not admin_shift_closed and pause_start_time:
                ...
                notify_subscribers("Адмін повернувся з перерви ▶")
            else:
            ...
            if admin_start_time and not admin_shift_closed and admin_shift_date == today:
                ...
                notify_subscribers(f"Зміна закрита ({today.strftime('%d.%m.%Y')}) ⏹")
            else:
    ...
    # Логіка користувачів
        ...
        if message.text == "🧐":
            ...
            if user_id in subs:
                ...
                bot.send_message(message.chat.id, "❌ Ви відписалися від сповіщень!")
            else:
                add_subscriber(user_id)
                bot.send_message(message.chat.id, "✅ Ви підписалися на сповіщення про статус зміни!")
